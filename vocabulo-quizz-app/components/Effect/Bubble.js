...
const Bubble = ({ size, color, duration, delay, opacity = 0.2 }) => {
    const adjustedSize = Math.min(Math.max(size, 20), 50);
    const initialTranslateX = Math.random() * (width - adjustedSize);
    const translateY = new Animated.Value(height);
    const translateX = new Animated.Value(initialTranslateX);
    const bubbleOpacity = new Animated.Value(opacity);
    ...
    }, [translateY, bubbleOpacity, duration, delay, adjustedSize, opacity]);
